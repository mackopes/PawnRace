#include "minimax_player_experimental.h"
...
Minimax_Player_Experimental::Minimax_Player_Experimental(tile color) : Player(color) {
  max_time_ = 5000;
  set_timer();
  timeout_flag_ = false;
  boardhash.clear();
...
Minimax_Player_Experimental::Minimax_Player_Experimental(tile color, long max_time) : Player(color) {
  max_time_ = max_time;
  set_timer();
  timeout_flag_ = false;
  boardhash.clear();
...
Move Minimax_Player_Experimental::get_move(Board board) {
  ll w, b, ep;
  best_eval_ = 0;
...
Move Minimax_Player_Experimental::minimax(ll attacker, ll deffender, ll ep) {
...
void Minimax_Player_Experimental::get_next_position(movetype movtp, ll move, ll attacker, ll deffender, ll ep, ll & new_att, ll & new_def, ll & new_ep) {
  switch (movtp) {
  case fwd:
...
Move Minimax_Player_Experimental::translate_move(ll move, movetype movtp) {
  if (color() == black) {
    switch (movtp) {
    case fwd:
 ...
 */
Move Minimax_Player_Experimental::minimax_start(ll attacker, ll deffender, ll ep, int max_depth) {
...
double Minimax_Player_Experimental::alphabeta_maximizing(ll attacker, ll deffender, ll ep, int cur_depth, int max_depth, double alpha, double beta) {
...
double Minimax_Player_Experimental::alphabeta_minimizing(ll attacker, ll deffender, ll ep, int cur_depth, int max_depth, double alpha, double beta) {
  if (max_depth != STARTDEPTH && (timeout() || timeout_flag_)) {
    timeout_flag_ = true;
    return -1;
  }
...
double Minimax_Player_Experimental::eval (ll attacker, ll deffender, int depth) {
  double e = 2.71828;
  double eval = eval_positions(attacker, deffender)/ (depth*10);
 ...
 */
double Minimax_Player_Experimental::eval_positions(ll attacker, ll deffender) {
  double att = 0;
  double def = 0;
  int multiplier = 1;
  int rowcount = 0;
